...
# =============================================================================
# get_centre_admin_emails / get_global_admin_emails
# =============================================================================
...
class GlobalAdminEmailsTests(SimpleTestCase):
    def test_only_admin_global_roles_contribute(self, mock_roles_db):
        mock_roles_db.list_roles.return_value = [
            admin_role(centre_id=mailer.GLOBAL_SCOPE, members=[{'email': 'super@example.com'}]),
            basic_role(centre_id=mailer.GLOBAL_SCOPE, members=[{'email': 'affiliate@example.com'}]),
        ]

        emails = mailer.get_global_admin_emails()

        self.assertEqual(emails, {'super@example.com'})
        mock_roles_db.list_roles.assert_called_once_with(mailer.GLOBAL_SCOPE)
...
class PermissionUpdatedEmailTests(SimpleTestCase):
    def test_centre_role_notifies_members_and_centre_admins(self, mock_centres_db, mock_roles_db, mock_send_mail):
        mock_centres_db.get_centre.return_value = {'id': CENTRE_ID, 'manager': {'email': 'manager@example.com'}}
        mock_roles_db.list_roles.return_value = []
    ...
    def test_global_role_notifies_members_and_global_admins(self, mock_centres_db, mock_roles_db, mock_send_mail):
        mock_roles_db.list_roles.return_value = [
            admin_role(centre_id=mailer.GLOBAL_SCOPE, members=[{'email': 'super@example.com'}]),
        ]
        role = basic_role(centre_id=mailer.GLOBAL_SCOPE, members=[{'email': 'affiliate@example.com'}])

        mailer.send_permission_updated_email(role, [('people.manage', {'visible': True, 'edit': True})])

        recipients = {c.kwargs['recipient_list'][0] for c in mock_send_mail.call_args_list}
        self.assertEqual(recipients, {'affiliate@example.com', 'super@example.com'})
